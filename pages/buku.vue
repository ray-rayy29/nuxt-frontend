    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
          ...
          <p class="text-slate-400 text-sm">Kelola data buku perpustakaan</p>
        </div>
        <button
          @click="openAddModal"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-slate-800/60 rounded-xl border border-slate-700/50 p-4">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-slate-400 text-xs">Total Buku</p>
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div class="relative bg-slate-800 rounded-xl w-full max-w-md max-h-[85vh] overflow-y-auto border border-slate-700">
        <div class="sticky top-0 bg-slate-800 p-4 border-b border-slate-700 flex justify-between items-center">
          <h2 class="text-lg font-semibold">{{ isEditing ? 'Edit Buku' : 'Tambah Buku' }}</h2>
...
const API_URL = `${config.public.apiBase}/keterangan`
...
const exportToExcel = () => {
  try {
    ...
    const fileName = `Data_Buku_${dateStr}.xlsx`
    
    // Download file
    XLSX.writeFile(wb, fileName)
    
    alert(`Berhasil mengeksport ${filteredData.value.length} data buku!`)
  } catch (err) {
